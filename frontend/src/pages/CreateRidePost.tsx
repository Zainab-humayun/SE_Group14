...
const CreateRidePost = () => {
  ...
  const [selectedFromLocation, setSelectedFromLocation] = useState<[number, number] | null>(null);
  const [selectedToLocation, setSelectedToLocation] = useState<[number, number] | null>(null);
  ...
  const formatLocationName = (location: Location) => {
    ...
    const notablePlaces = ['LUMS', 'Hafeez Center', 'DHA', 'Model Town', 'Gulberg'];
  ...
  const handlePostClick = async (e: any) => {
    e.preventDefault();
    const pickLocation = fromSuggestions.find(loc => loc.display_name === fromQuery) 
      ? formatLocationName(fromSuggestions.find(loc => loc.display_name === fromQuery)!) 
      : fromQuery;
    
    const dropLocation = toSuggestions.find(loc => loc.display_name === toQuery) 
      ? formatLocationName(toSuggestions.find(loc => loc.display_name === toQuery)!) 
      : toQuery;
    ...
    <div className={`flex flex-col lg:flex-row min-h-screen ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
          ...
          <form className="space-y-6" onSubmit={handlePostClick}>
            ...
            <div className="space-y-4">
              <div className="relative">
                <label className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>From</label>
                <div className={`flex items-center px-4 py-3 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}>
                ...
                {fromSuggestions.length > 0 && (
                  <ul className={`absolute w-full mt-1 rounded-lg shadow-lg z-20 overflow-hidden ${darkMode ? "bg-gray-700" : "bg-white"}`}>
                    {fromSuggestions.map((place: Location) => (
                      ...
                      >
                        <p className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>{formatLocationName(place)}</p>
                        <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{place.display_name.split(',').slice(0, 3).join(',')}</p>
                      </li>
                    ))}
                  </ul>
                )}
              ...
              <div className="relative">
                <label className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>To</label>
                <div className={`flex items-center px-4 py-3 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}>
                  <div className={`w-3 h-3 rounded-full mr-3 ${darkMode ? "bg-red-400" : "bg-red-500"}`}></div>
                  <input
                    type="text"
              ...
              <div className="flex space-x-2">
                {[1, 2, 3, 4].map((num) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg ${passengers === num 
      ...
      <div className="lg:w-2/3 h-[500px] lg:h-auto sticky overflow-hidden rounded-2xl border-2 transition-all duration-300 bg-white dark:bg-gray-800">
        {selectedFromLocation || selectedToLocation ? (
          ...
          <div className={`absolute inset-0 flex items-center justify-center ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}>
            <p className={`text-center px-4 ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
