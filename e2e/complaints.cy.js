  ...
  describe('Campus Caravan Login and Search Test', () => {
    it('Logs in - searches - adds complaint', () => {
      ...
      cy.get('svg') // FontAwesome renders icons as <svg>
        ...
        .eq(2) // May need to fine-tune this depending on the order
        .click();
      cy.wait(2000);
      // Now the SearchBarSmall component should be visible if isSearchOpen = true
      cy.get('input[placeholder="Search users by name or student ID..."]').type('Minhaz');
      cy.wait(5000);
      cy.get('a[data-discover="true"]').eq(3).click({ force: true });
      cy.wait(1000);
      cy.contains('button', 'Add').click();
      cy.wait(1000);
      cy.get('textarea#complain')
        .clear() // Clear any existing text
        .type('This is my complaint text.') // Replace with your desired text
